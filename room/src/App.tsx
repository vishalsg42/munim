    ...
    <div className="min-h-full font-sans antialiased">
      <header className="flex items-center justify-between border-b border-white/6 px-8 py-4">
        ...
        <span className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-white/35">
          <i className={`h-1.5 w-1.5 rounded-full ${state.connected ? "bg-emerald-400" : "bg-white/20"}`} />
...
function Empty() {
  return (
    <div className="grid place-items-center px-8 py-32 text-center">
      <div>
        <p className="text-white/45">No launch running.</p>
...
function Finding({ event }: { event: LaunchEvent }) {
  ...
  return (
    <section className="mt-8 rounded-lg border border-rose-500/40 bg-rose-500/[0.06] p-6">
      <p className="text-[11px] uppercase tracking-[0.2em] text-rose-300/70">
        Needs attention
      </p>
      <p className="mt-2 text-lg leading-snug text-rose-50">{event.human_text}</p>
      {evidence && (
        <pre className="mt-4 overflow-x-auto rounded border border-white/8 bg-black/40 p-3 font-mono text-[11px] leading-relaxed text-white/60">
{evidence}
        </pre>
      )}
      {resolver && (
...
function Confirm({ event }: { event: LaunchEvent }) {
  return (
    <section className="mt-6 rounded-lg border border-amber-400/40 bg-amber-400/[0.06] p-6">
      <p className="text-[11px] uppercase tracking-[0.2em] text-amber-300/70">
        Waiting for you
      </p>
      <p className="mt-2 text-[15px] text-amber-50">{event.human_text}</p>
      <p className="mt-3 text-[12px] text-white/45">
        This changes <span className="text-white/80">{event.client}</span>&rsquo;s
        live DNS. It is their account, not yours.
      </p>
      <button className="mt-4 rounded-md bg-amber-400 px-4 py-2 text-[13px] font-medium text-black transition-colors hover:bg-amber-300">
...
function Log({ events }: { events: LaunchEvent[] }) {
  ...
  return (
    <section className="mt-10 border-t border-white/6 pt-4">
      <div className="max-h-40 space-y-1 overflow-y-auto font-mono text-[11px] text-white/35">
        {events.map((e) => (
          <p key={e.seq}>
            <span className="text-white/20">{String(e.seq).padStart(3, "0")}</span>{" "}
            <span className="text-white/25">{e.stage}</span> {e.human_text}
          </p>
        ))}
        <div ref={end} />
      </div>
