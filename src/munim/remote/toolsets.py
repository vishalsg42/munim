...
def toolset_for(client: str, provider: str, *, backend=None) -> MCPClient:
...
def toolsets_for(clients: list[str], provider: str, *, backend=None) -> list[MCPClient]:
    ...
    for client in clients:
        ...
        seen[prefix] = client
    return [toolset_for(c, provider, backend=backend) for c in clients]
